 ...
 *
 * For PowerPC, we use the timebase counter returned by get_tb() for this
 * purpose.
 */

#ifndef __POWERPC_SX_TIME_H
#define __POWERPC_SX_TIME_H
 ...
 * (NSEC_PER_SEC / ppc_tb_freq) * get_tb()
 * i.e., ns_per_tick * ticks_since_boot.
 * But if ppc_tb_freq is 66666667, then
 * (NSEC_PER_SEC / ppc_tb_freq) becomes 14ns (incorrect) instead of
...
{
	return div_u64((NSEC_PER_SEC / 1000) * get_tb(),
		       ppc_tb_freq) * 1000;
}

#endif /* __POWERPC_SX_TIME_H */
