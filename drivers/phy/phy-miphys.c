...
static int miphys_init_sgmii(struct miphys_phy *miphy_phy)
{
	...
	if (cfg->inverted_polarity)
		writel_relaxed(0x74, base + MIPHYS_DS(0, MIPHYS_CONTROL));
...
static int miphys_init_mdml(struct miphys_phy *miphy_phy)
{
	...
	if (miphy_phy->args1 == PHY_MDML_FREQ_6Gbps) {
				...
				struct of_phandle_args *args)
{
	...
	if (miphy_phy->type != PHY_NONE &&
	    ...
	    (miphy_phy->args1 != (args->args_count == 2 ? args->args[1] : 0)))) {
