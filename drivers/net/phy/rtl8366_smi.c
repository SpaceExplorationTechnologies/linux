...
static void rtl8366_debugfs_init(struct rtl8366_smi *smi)
{
	...
	node = debugfs_create_x16("reg", 0644, root, &smi->dbg_reg);
	if (!node) {
	...
	node = debugfs_create_file("val", 0644, root, smi, &fops_rtl8366_regs);
	if (!node) {
		dev_err(smi->parent, "Creating debugfs file '%s' failed\n",
			"val");
		return;
	...
	node = debugfs_create_file("vlan_mc", 0400, root, smi,
	...
	node = debugfs_create_u8("vlan_4k_page", 0644, root,
				 &smi->dbg_vlan_4k_page);
	if (!node) {
		dev_err(smi->parent, "Creating debugfs file '%s' failed\n",
			"vlan_4k_page");
	...
	node = debugfs_create_file("vlan_4k", 0400, root, smi,
	...
	node = debugfs_create_file("pvid", 0400, root, smi, &fops_rtl8366_pvid);
	if (!node) {
		dev_err(smi->parent, "Creating debugfs file '%s' failed\n",
			"pvid");
		return;
	...
	node = debugfs_create_file("mibs", 0400, smi->debugfs_root, smi,
				   &fops_rtl8366_mibs);
	if (!node)
		dev_err(smi->parent, "Creating debugfs file '%s' failed\n",
			"mibs");
