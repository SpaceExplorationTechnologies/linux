...
#define RTL8367RB_DRIVER_NAME	"rtl8367rb"

#define RTL8367_RESET_DELAY	1000	/* msecs*/
#define RTL8367_PHY_REG_RETRY_CNT         5 /* 1us between retries */
				...
				u32 phy_addr, u32 phy_reg, u32 *val)
{
	...
	timeout = RTL8367_PHY_REG_RETRY_CNT;
	do {
		REG_RD(smi, RTL8367_IA_STATUS_REG, &data);
...
static int rtl8367_init_regs0(struct rtl8366_smi *smi, unsigned int mode)
{
	const struct rtl8367_initval *initvals;
	int count;
	...
	default:
		dev_err(smi->parent, "%s: unknown mode %u\n", __func__, mode);
		return -ENODEV;
	}
...
static int rtl8367_init_regs1(struct rtl8366_smi *smi, unsigned int mode)
{
	const struct rtl8367_initval *initvals;
	int count;
...
static int rtl8367_init_regs2(struct rtl8366_smi *smi, unsigned int mode)
{
	const struct rtl8367_initval *initvals;
	int count;
...
static int rtl8367_init_regs(struct rtl8366_smi *smi)
{
	...
	case 1:
		...
		if (data == 0x94eb) {
			...
			dev_err(smi->parent, "unknown phy data %04x\n", data);
			return -ENODEV;
		}
...
{
	int timeout = 10000 / RTL8367_RESET_DELAY; /* Arbitrary 10 seconds */
	int err;
	u32 data;
				  ...
				  enum rtl8367_extif_mode mode)
{
	...
	/* set port mode */
	switch (mode) {
	case RTL8367_EXTIF_MODE_SGMII:
		REG_RMW(smi, RTL8367C_REG_SDS_MISC,
			RTL8367C_CFG_MAC8_SEL_SGMII_MASK,
			BIT(RTL8367C_CFG_MAC8_SEL_SGMII_OFFSET));
		REG_RMW(smi, RTL8367C_REG_SDS_MISC,
			RTL8367C_CFG_MAC8_SEL_HSGMII_MASK, 0);
		...
		REG_WR(smi, RTL8367C_REG_SDS_INDACS_DATA, 0x7180);
		REG_WR(smi, RTL8367C_REG_SDS_INDACS_ADR, 0x0002);
		REG_WR(smi, RTL8367C_REG_SDS_INDACS_CMD, 0x00C0);

		/* All settings done after the switch-case are only for
		 * NON SGMII mode hence we return from the function here
		 */
		return 0;
...
static int rtl8367_extif_set_force_sgmii(struct rtl8366_smi *smi, int id,
					 struct rtl8367_port_ability *pa)
{
	u32 mask;
	u32 val;
	...
	mask = (RTL8367C_CFG_SGMII_RXFC_MASK |
		RTL8367C_CFG_SGMII_TXFC_MASK |
		RTL8367C_CFG_SGMII_FDUP_MASK |
		RTL8367C_CFG_SGMII_LINK_MASK |
		RTL8367C_CFG_SGMII_SPD_MASK);

	val = pa->speed << RTL8367C_CFG_SGMII_SPD_OFFSET;
	val |= pa->txpause ? BIT(RTL8367C_CFG_SGMII_TXFC_OFFSET) : 0;
	val |= pa->rxpause ? BIT(RTL8367C_CFG_SGMII_RXFC_OFFSET) : 0;
	val |= pa->link ? BIT(RTL8367C_CFG_SGMII_LINK_OFFSET) : 0;
	val |= pa->duplex ? BIT(RTL8367C_CFG_SGMII_FDUP_OFFSET) : 0;

	REG_RMW(smi, RTL8367C_REG_SDS_MISC, mask, val);
					 ...
					 unsigned int rxdelay)
{
	...
	val = rxdelay | (txdelay << RTL8367_EXT_RGMXF_TXDELAY_SHIFT);
			      ...
			      struct rtl8367_extif_config *cfg)
{
	...
	if (mode != RTL8367_EXTIF_MODE_DISABLED) {
		...
		if (mode == RTL8367_EXTIF_MODE_SGMII) {
			err = rtl8367_extif_set_force_sgmii(smi, id,
							    &cfg->ability);
			if (err)
				return err;
		} else {
			err = rtl8367_extif_set_rgmii_delay(smi, id,
							    cfg->txdelay,
							    cfg->rxdelay);
			if (err)
				return err;
...
static void rtl8367_probe_config_dt(struct rtl8366_smi *smi)
{
	...
	mac = of_property_read_bool(np, "extif0");
	if (mac) {
		pdata->extif0_cfg = devm_kzalloc(smi->parent,
						 sizeof(struct
							rtl8367_extif_config),
						 GFP_KERNEL);
		of_property_read_u32(np, "extif0-mode",
				     &pdata->extif0_cfg->mode);
		of_property_read_u32(np, "extif0-txdelay",
				     &pdata->extif0_cfg->txdelay);
		of_property_read_u32(np, "extif0-rxdelay",
				     &pdata->extif0_cfg->rxdelay);
		of_property_read_u32(np, "extif0-force_mode",
				     &pdata->extif0_cfg->ability.force_mode);
		of_property_read_u32(np, "extif0-txpause",
				     &pdata->extif0_cfg->ability.txpause);
		of_property_read_u32(np, "extif0-rxpause",
				     &pdata->extif0_cfg->ability.rxpause);
		of_property_read_u32(np, "extif0-link",
				     &pdata->extif0_cfg->ability.link);
		of_property_read_u32(np, "extif0-duplex",
				     &pdata->extif0_cfg->ability.duplex);
		of_property_read_u32(np, "extif0-speed",
				     &pdata->extif0_cfg->ability.speed);
		of_property_read_u32(np, "extif0-nway",
				     &pdata->extif0_cfg->ability.nway);
	}
	mac = of_property_read_bool(np, "extif1");
	if (mac) {
		pdata->extif1_cfg = devm_kzalloc(smi->parent,
						 sizeof(struct
							rtl8367_extif_config),
						 GFP_KERNEL);
		of_property_read_u32(np, "extif1-mode",
				     &pdata->extif1_cfg->mode);
		of_property_read_u32(np, "extif1-txdelay",
				     &pdata->extif1_cfg->txdelay);
		of_property_read_u32(np, "extif1-rxdelay",
				     &pdata->extif1_cfg->rxdelay);
		of_property_read_u32(np, "extif1-force_mode",
				     &pdata->extif1_cfg->ability.force_mode);
		of_property_read_u32(np, "extif1-txpause",
				     &pdata->extif1_cfg->ability.txpause);
		of_property_read_u32(np, "extif1-rxpause",
				     &pdata->extif1_cfg->ability.rxpause);
		of_property_read_u32(np, "extif1-link",
				     &pdata->extif1_cfg->ability.link);
		of_property_read_u32(np, "extif1-duplex",
				     &pdata->extif1_cfg->ability.duplex);
		of_property_read_u32(np, "extif1-speed",
				     &pdata->extif1_cfg->ability.speed);
		of_property_read_u32(np, "extif1-nway",
				     &pdata->extif1_cfg->ability.nway);
	}
	mac = of_property_read_bool(np, "extif2");
	if (mac) {
		pdata->extif2_cfg = devm_kzalloc(smi->parent,
						 sizeof(struct
							rtl8367_extif_config),
						 GFP_KERNEL);
		of_property_read_u32(np, "extif2-mode",
				     &pdata->extif2_cfg->mode);
		of_property_read_u32(np, "extif2-txdelay",
				     &pdata->extif2_cfg->txdelay);
		of_property_read_u32(np, "extif2-rxdelay",
				     &pdata->extif2_cfg->rxdelay);
		of_property_read_u32(np, "extif2-force_mode",
				     &pdata->extif2_cfg->ability.force_mode);
		of_property_read_u32(np, "extif2-txpause",
				     &pdata->extif2_cfg->ability.txpause);
		of_property_read_u32(np, "extif2-rxpause",
				     &pdata->extif2_cfg->ability.rxpause);
		of_property_read_u32(np, "extif2-link",
				     &pdata->extif2_cfg->ability.link);
		of_property_read_u32(np, "extif2-duplex",
				     &pdata->extif2_cfg->ability.duplex);
		of_property_read_u32(np, "extif2-speed",
				     &pdata->extif2_cfg->ability.speed);
		of_property_read_u32(np, "extif2-nway",
...
static int rtl8367_setup(struct rtl8366_smi *smi)
{
	...
	/* No whole init_regs done for the 8366sc */
	if (!of_device_is_compatible(np, "realtek,rtl8366sc")) {
				   ...
				   int port, unsigned long long *val)
{
	...
	BUILD_BUG_ON(ARRAY_SIZE(rtl8367_mib_counters) != RTL8367_MIB_COUNT);
...
static int rtl8367_is_vlan_valid(struct rtl8366_smi *smi, unsigned int vlan)
{
	unsigned int max = RTL8367_NUM_VLANS;
...
static struct rtl8366_smi_ops rtl8366sc_smi_ops = {
	.reset_chip	= rtl8367_reset_chip,
	.setup		= rtl8367_setup,
...
static int rtl8367_probe(struct platform_device *pdev)
{
	...
	if (of_device_is_compatible(pdev->dev.of_node, "realtek,rtl8366sc")) {
		/* No whole init_regs done for the 8366sc */
		smi->ops = &rtl8366sc_smi_ops;
	} else {
...
MODULE_DESCRIPTION("Realtek RTL8367R/M ethernet switches driver");
