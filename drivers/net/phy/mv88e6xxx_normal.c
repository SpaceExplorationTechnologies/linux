					...
					struct device_node *node)
{
	int err;
	u16 cmode;
	int phy_mode = of_get_phy_mode(node);
	...
	case PHY_INTERFACE_MODE_RGMII_RXID:
		if (cmode != PORT_STATUS_CMODE_RGMII) {
			dev_info(chip->dev, "Port and CMODE mismatch.  CMODE: %d OF: %s\n",
				 cmode, phy_modes(phy_mode));
		}
		break;
...
static int mv88e6xxx_detect(struct mv88e6xxx_chip *chip)
{
	...
	if (prod_num != chip->info->prod_num) {
		dev_err(chip->dev, "Unsupported prod_num %x detected\n",
			prod_num);
		return -ENODEV;
	}
