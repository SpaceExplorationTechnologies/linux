...
#ifdef CONFIG_SPACEX
	/*
	 * Whether to inject "soft" errors or not. Note that soft
	 * errors do not excercise any of the CPU error handling, just
	 * the driver's.
	 */
	bool inject_soft_errors;

	/*
	 * The number of soft errors injected since the last polling
	 * by the driver.
	 */
	unsigned int num_soft_errors_injected;
#endif /* !CONFIG_SPACEX */
};
...
{
#ifdef CONFIG_SPACEX
	struct cortex_arm64_edac *drv = arg;
#endif /* CONFIG_SPACEX */
	int cpu, partnum, way;
...
#else /* !CONFIG_SPACEX */
	if (!CPUMERRSR_EL1_VALID(val) && !drv->inject_soft_errors)
#endif /* CONFIG_SPACEX */
		return;
	...
	edac_device_handle_ce(drv->edac_ctl, 0, 0, drv->edac_ctl->ctl_name);

	drv->num_soft_errors_injected += drv->inject_soft_errors;
#endif /* CONFIG_SPACEX */
...
static void parse_l2merrsr(void *arg)
{
...
#else /* !CONFIG_SPACEX */
	if (!L2MERRSR_EL1_VALID(val) && !drv->inject_soft_errors)
#endif /* CONFIG_SPACEX */
		return;
	...
	edac_device_handle_ce(drv->edac_ctl, 0, 1, drv->edac_ctl->ctl_name);
...
#ifdef CONFIG_SPACEX
	struct cortex_arm64_edac *drv = edac_ctl->pvt_info;

	if (drv->num_soft_errors_injected) {
		drv->inject_soft_errors = false;
		drv->num_soft_errors_injected = 0;
	}
#endif /* CONFIG_SPACEX */
...
#else /* !CONFIG_SPACEX */
		smp_call_function_single(cpu, parse_cpumerrsr, drv, 0);
#endif /* CONFIG_SPACEX */
...
#else /* !CONFIG_SPACEX */
		smp_call_function_any(&cluster_mask, parse_l2merrsr, drv, 0);
#endif /* CONFIG_SPACEX */
	}
	put_online_cpus();
}
...
#ifdef CONFIG_SPACEX
static ssize_t cortexa53_edac_inject_soft_show(struct edac_device_ctl_info *dci,
                                               char *data)
{
	struct cortex_arm64_edac *drv = dci->pvt_info;

	return sprintf(data, "%u", drv->inject_soft_errors);
}

static ssize_t cortexa53_edac_inject_soft_store(
    struct edac_device_ctl_info *dci, const char *data, size_t count)
{
	struct cortex_arm64_edac *drv = dci->pvt_info;

	if (isdigit(*data)) {
		drv->inject_soft_errors = !!simple_strtoul(data, NULL, 0);
		return count;
	}
	return 0;
}
...
static struct edac_dev_sysfs_attribute cortexa53_edac_sysfs_attributes[] = {
	...
	{
		.attr = {
			.name = "inject_Soft_Error",
			.mode = (S_IRUGO | S_IWUSR)
		},
		.show = cortexa53_edac_inject_soft_show,
		.store = cortexa53_edac_inject_soft_store},
#endif /* !CONFIG_SPACEX */
	/* End of list */
	{
		.attr = {.name = NULL}
...
static int cortex_arm64_edac_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(drv->edac_ctl))
		return -ENOMEM;
#else /* !CONFIG_SPACEX */
	edac_ctl = edac_device_alloc_ctl_info(sizeof(*drv), "cpu", 1, "L", 2,
					      1, NULL, 0,
					      edac_device_alloc_index());
	if (IS_ERR(edac_ctl))
		return -ENOMEM;
	drv = edac_ctl->pvt_info;
	drv->edac_ctl = edac_ctl;
#endif /* CONFIG_SPACEX */

#ifdef CONFIG_SPACEX
	/*
	 * Do not log correctable errors: displaying the message could
	 * cause a delay in a real-time thread and cause a desync.
	 */
	drv->edac_ctl->log_ce = 0;
