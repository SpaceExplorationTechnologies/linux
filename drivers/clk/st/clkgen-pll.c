...
#define C28_CP_MASK (0x1f)
#define C28_MODE_MASK (0x1)
#define C28_DITH_MASK (0x3)
#define C28_FRAC_MASK (0xffff)
...
struct clkgen_pll_data {
	...
	struct clkgen_field mode;
	struct clkgen_field frac;
	spinlock_t *lock;
...
static const struct clk_ops stm_pll5000c28_ops;
...
static struct clkgen_pll_data st_pll5000c28_cx_0 = {
	/* C0 PLL0 */
	.pdn_status = CLKGEN_FIELD(0x2a0,   0x1,            8),
	.pdn_ctrl   = CLKGEN_FIELD(0x2a0,   0x1,            8),
	.locked_status  = CLKGEN_FIELD(0x2a0,   0x1,            24),
	.ndiv       = CLKGEN_FIELD(0x2a4,   C28_NDIV_MASK,      16),
	.idf        = CLKGEN_FIELD(0x2a4,   C28_IDF_MASK,       0x0),
	.mode       = CLKGEN_FIELD(0x2a8,   C28_MODE_MASK,      21),
	.frac       = CLKGEN_FIELD(0x2a8,   C28_FRAC_MASK,      0),
	.num_odfs = 1,
	/* odf_gate not present */
	.odf        = { CLKGEN_FIELD(0x2b4, C28_ODF_MASK,       0), },
	.ops        = &stm_pll5000c28_ops,
};

static struct clkgen_pll_data st_pll5000c28_cx_1 = {
	/* C0 PLL1 */
	.pdn_status = CLKGEN_FIELD(0x2c8,   0x1,            8),
	.pdn_ctrl   = CLKGEN_FIELD(0x2c8,   0x1,            8),
	.locked_status  = CLKGEN_FIELD(0x2c8,   0x1,            24),
	.ndiv       = CLKGEN_FIELD(0x2cc,   C28_NDIV_MASK,      16),
	.idf        = CLKGEN_FIELD(0x2cc,   C28_IDF_MASK,       0x0),
	.mode       = CLKGEN_FIELD(0x2d0,   C28_MODE_MASK,      21),
	.frac       = CLKGEN_FIELD(0x2d0,   C28_FRAC_MASK,      0),
	.num_odfs = 1,
	/* odf_gate not present */
	.odf        = { CLKGEN_FIELD(0x2dc, C28_ODF_MASK,       0), },
	.ops        = &stm_pll5000c28_ops,
};
...
static int vclk_pll5000c28_get_rate(unsigned long input, struct stm_pll *pll,
				    unsigned long *rate)
{
	u64 res;
	unsigned short odf;
	...
	if (!pll->idf)
		pll->idf = 1;
	if (pll->odf > 5)
		pll->odf = 5; /* Max reg value */
	odf = 1 << pll->odf; /* 1, 2, 4, 8, 16, 32 */

	/*
	 * PHI = (INFF * LDF) / (2 * ODF * IDF)
	 * INTEGER mode:
	 *	LDF = 2 * NDIV
	 *	=> PHI = (INFF * NDIV) / (ODF * IDF)
	 * FRACTIONAL mode
	 *	LDF = 2 * (NDIV + FRACT/2P16 + 1/2P17) (SSCG_CONTROL = L)
	 *	LDF = 2 * (NDIV + FRACT/2P16) (SSCG_CONTROL = H)
	 */
	if (!pll->mode) { /* INTEGER mode */
		res = (u64)input * pll->ndiv;
		*rate = div64_u64(res, (u64)(odf * pll->idf));
	} else { /* FRACTIONAL mode (SSCG_CONTROL=H) */
		res = ((u64)(BIT(16) * pll->ndiv) + pll->frac) * input;
		*rate = div64_u64(res, (u64)(BIT(16) * pll->idf * odf));
	}

	return 0;
}

static int vclk_pll5000c28_get_params(unsigned long input, unsigned long output,
				      struct stm_pll *pll, long *rate)
{
	unsigned long i, n, o; /* IDF, NDIV, ODF formula values */
	unsigned long deviation = ~0;
	unsigned long new_freq, new_deviation;
	struct stm_pll pll_tmp = {
		.mode = pll->mode
	};
	int i_loops;
	u64 f, tmp;
	unsigned long fvcoby2;

	if (!pll->mode) /* INTEGER */
		return -1; /* Unsupported */

	/* Computing ODF: 1, 2, 4, 8, 16 or 32 */
	for (o = 1, pll->odf = 0, fvcoby2 = output; o < 32; ) {
		if (fvcoby2 >= 1250000000)
			break; /* FVCOby2 in range */
		(pll->odf)++;
		o <<= 1;
		fvcoby2 = o * output;
	}
	...
	/* FRACTIONAL with SSCG ON */
	for (; (i <= 7) && i_loops; i++, i_loops--) {
		for (n = 104; n >= 31; n--) {
			/* ODF * IDF * PHI */
			f = ((u64)output * i * o);
			tmp = ((u64)n * input); /* INFF * NDIV */
			if (tmp > f)
				continue; /* Invalid */

			f -= tmp;
			f = div64_u64(f << 16, input);
			...
			/* Check f & f+1 to cover division rounding */
			for (tmp = 0; tmp <= 1; tmp++) {
				pll_tmp.frac = f + tmp;
				vclk_pll5000c28_get_rate(input, &pll_tmp,
							 &new_freq);
...
no_deviation:
	return vclk_pll5000c28_get_rate(input, pll, rate);
}

static unsigned long recalc_stm_pll5000c28(struct clk_hw *hw,
					   unsigned long parent_rate)
{
	struct clkgen_pll *pll = to_clkgen_pll(hw);
	struct stm_pll params = {};
	...
	params.mode = CLKGEN_READ(pll, mode);
	params.frac = CLKGEN_READ(pll, frac);

	vclk_pll5000c28_get_rate(parent_rate, &params, &rate);
...
static long round_rate_stm_pll5000c28(struct clk_hw *hw, unsigned long rate,
				      unsigned long *prate)
{
	struct stm_pll params = { .idf = 0xff };

	if (vclk_pll5000c28_get_params(*prate, rate, &params, &rate)) {
		pr_debug("%s: %s rate %ld Invalid\n", __func__,
...
static int set_rate_stm_pll5000c28(struct clk_hw *hw, unsigned long rate,
				   unsigned long parent_rate)
{
	struct clkgen_pll *pll = to_clkgen_pll(hw);
	struct stm_pll params = { .idf = 0xff,
	...
	if (vclk_pll5000c28_get_params(parent_rate, rate, &params, &hwrate)) {
		pr_debug("%s: %s rate %ld Invalid\n", __func__,
			 clk_hw_get_name(hw), rate);
		return -EINVAL;
	}

	pr_debug("%s: %s new rate %ld [ndiv=0x%x] [idf=0x%x] [mode=0x%x] [frac=0x%x]\n",
		 __func__, clk_hw_get_name(hw),
		 hwrate, (unsigned int)params.ndiv,
		 (unsigned int)params.idf,
		 (unsigned int)params.mode,
		 (unsigned int)params.frac);
	...
	pll->ndiv = params.ndiv;
	pll->idf = params.idf;
	pll->mode = params.mode;
	pll->frac = params.frac;

	__clkgen_pll_disable(hw);

	CLKGEN_WRITE(pll, ndiv, pll->ndiv);
	CLKGEN_WRITE(pll, idf, pll->idf);
	CLKGEN_WRITE(pll, mode, pll->mode);
	CLKGEN_WRITE(pll, frac, pll->frac);
...
static const struct clk_ops stm_pll5000c28_ops = {
	...
	.disable	= clkgen_pll_disable,
	.is_enabled	= clkgen_pll_is_enabled,
	.recalc_rate	= recalc_stm_pll5000c28,
	.round_rate	= round_rate_stm_pll5000c28,
	.set_rate	= set_rate_stm_pll5000c28,
};
...
static void __init clkgen_c28_pll5kcx0_setup(struct device_node *np)
{
		clkgen_c32_pll_setup(np,
			(struct clkgen_pll_data *)&st_pll5000c28_cx_0);
}
CLK_OF_DECLARE(c28_pll5k0, "st,stigllcff-clkgen-pll5k0", clkgen_c28_pll5kcx0_setup);

static void __init clkgen_c28_pll5kcx1_setup(struct device_node *np)
{
		clkgen_c32_pll_setup(np,
			(struct clkgen_pll_data *)&st_pll5000c28_cx_1);
}
CLK_OF_DECLARE(c28_pll5k1, "st,stigllcff-clkgen-pll5k1", clkgen_c28_pll5kcx1_setup);
