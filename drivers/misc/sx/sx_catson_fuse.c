...
static int sx_catson_fuse_cache(struct catson_fuses *cfuse)
{
	...
	cfuse->cache.cert_ctr.trusted_boot_fw = res.a1;
...
/**
 * show_rotpk_idx() - displays the rotpk skey.
 *
 * @dev the ID core device.
 * @attr the device_attribute we are using (ignored).
 * @buf the kernel-buffer to fill with output.
 * @idx the key index to read
 *
 * Return: the number of bytes written to buf, or a negative value on error.
 */
static ssize_t show_key_raw(struct device *dev, struct device_attribute *attr,
			    char *buf, void *fuse)
{
	memcpy(buf, fuse, CATSON_KEY_SIZE);

	return CATSON_KEY_SIZE;
}
...
/**
 * run_burn_skey() - burns a new skey into fuses.
 *
 * @dev the ID core device.
 * @attr the device_attribute we are using (ignored).
 * @buf the kernel-buffer filled with input.
			     ...
			     const char *buf, size_t count)
{
	...
	arm_smccc_smc(CATSON_SIP_SVC_BLOW_SKEY, handle, count, 0, 0, 0, 0, 0,
		      &res);
...
/**
 * run_burn_rkey() - burns the ROM rkey into fuses.
 *
 * @dev the ID core device.
 * @attr the device_attribute we are using (ignored).
 * @buf the kernel-buffer filled with input.
			     ...
			     const char *buf, size_t count)
{
	...
	arm_smccc_smc(CATSON_SIP_SVC_BLOW_RKEY, handle, count, 0, 0, 0, 0, 0,
		      &res);
			     ...
			     const char *buf, size_t count)
{
	...
	if ((tb_fw_cert != cfuse->cache.cert_ctr.soc_fw_key) ||
