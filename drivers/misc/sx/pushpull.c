...
/**
 * struct eut_pull_descriptor - hardware transmit descriptor.
 * @link_id:	identifier used for source identification.
 * @reserved0:	reserved region.
 * @address:	physical address of the data to transmit.
 * @length:	length of data to send in bytes.
 */
struct eut_pull_descriptor {
	u16 link_id;
	u8 reserved0[6];
	u32 address;
	u32 length;
} __packed;
#define EUT_PULL_DESCRIPTOR_SIZE 16
 ...
 * See https://stash/projects/FPGASAT/repos/gnet_fpga/browse/gnet_top/sub/gnet_core/src/gnet_core.sv#92
 */
...
union pp_descriptor {
	...
	union {
		...
		struct eut_pull_descriptor eut;
 ...
 * @dynamic_pull_buffers:	Set true to prevent preallocated pull buffers.
 */
struct pp_desc_info {
	const char *name;
				    ...
				    struct pp_fifo_entry *entry)
{
	...
	entry->priv = skb;
	entry->buffer_phys = dma_map_single(&chan->dev->dev, skb->data,
...
static int pp_push_start_receive(struct pp_push_channel *pchan)
{
	...
	while (push_can_fill_used_fifo(chan)) {
		...
		if (pchan->push_prepare_desc &&
		    WARN_ONCE(pchan->push_prepare_desc(pchan,
						       entry),
	...
	for (i = 0; i < chan->hw_entries.fifo_count; i++) {
		if (!chan->dynamic_buffers) {
			dma_pool_free(chan->hw_entries.buffer_pool,
				      chan->hw_entries.fifo_entries[i].buffer,
				  chan->hw_entries.fifo_entries[i].buffer_phys);
		}

		dma_pool_free(chan->hw_entries.desc_pool,
			      chan->hw_entries.fifo_entries[i].desc,
			      chan->hw_entries.fifo_entries[i].desc_phys);
	}
}
			       ...
			       irq_handler_t irq_handler)
{
	...
	for (i = 0; i < chan->hw_entries.fifo_count; i++) {
		chan->hw_entries.fifo_entries[i].desc =
		    dma_pool_zalloc(chan->hw_entries.desc_pool, GFP_KERNEL,
				    &chan->hw_entries.fifo_entries[i].desc_phys);
		if (!chan->hw_entries.fifo_entries[i].desc) {
			rc = -ENOMEM;
		...
		if (!chan->dynamic_buffers) {
			chan->hw_entries.fifo_entries[i].buffer =
			    dma_pool_zalloc(chan->hw_entries.buffer_pool,
					    GFP_KERNEL,
					    &chan->hw_entries.fifo_entries[i].buffer_phys);
			if (!chan->hw_entries.fifo_entries[i].buffer) {
				rc = -ENOMEM;
				goto dma_alloc_failed;
			}
		}

		INIT_LIST_HEAD(&chan->hw_entries.fifo_entries[i].list);

		/*
		 * Add to free fifo. Since this is init, defer signalling the
		 * wait queue.
		 */
		list_add(&chan->hw_entries.fifo_entries[i].list,
			 &chan->free.fifo);
		chan->free.depth++;
	}
 ...
 * @entry:	The software construct assocated with the hardware message.
 */
				 ...
				 struct pp_fifo_entry *entry)
{
	...
	for (i = 0; i < MAX_LINKID_DFS_COUNT; i++) {
		if (dchan->link_id[i] == desc->link_id) {
			idx = i;
			break;
		}
	}

	if (WARN_ONCE(idx == -1, "Invalid linkid %d detected\n",
		      desc->link_id)) {
	 ...
	 */
	WARN_ONCE(entry->data_len,
		  "Entry has non-zero length %zu\n", entry->data_len);
	entry->data_len = desc->payload_length;
	 ...
	 */
	if (WARN_ONCE(entry->data_len > chan->buffer_size,
	 ...
	 * to zero.
	 */
	if (WARN_ONCE(desc->dma_status & RX_DESC_DMA_STATUS_ERR_PACKET,
	 ...
	 */
	spin_lock_irqsave(&chan->lock, flags);
	list_add_tail(&entry->list, &dchan->user_queue[idx].fifo);
	dchan->user_queue[idx].depth++;
	spin_unlock_irqrestore(&chan->lock, flags);
			       ...
			       struct pp_fifo_entry *entry)
{
	...
	WARN_ONCE(entry->data_len,
		  "Entry has non-zero length %zu\n", entry->data_len);
	/*
	 * Read the actual packet size from the DMA descriptor.
	 */
				 ...
				 struct pp_fifo_entry *entry)
{
	...
	/*
	 * Read the actual packet size from the DMA descriptor.
	 * The "reason" field is currently not exposed to user space.
	 */
	entry->data_len = desc->length;
...
/**
 * gnpunet_push_handle_pkt() - Handler for incoming Ethernet packets from GNET
 *    NPU.
 * @pchan:	The push channel a packet was received on.
 * @entry:	The entry received from hardware.
 */
static void gnpunet_push_handle_pkt(struct pp_push_channel *pchan,
				    struct pp_fifo_entry *entry)
{
	struct npu_push_descriptor *desc =
		(struct npu_push_descriptor *)entry->desc;
...
static int pp_net_napi_poll(struct napi_struct *napi, int budget)
{
	...
	while (packets_handled < budget &&
		...
		if (WARN_ONCE(!skb, "Entry has no SKB allocated\n")) {
			dchan->netdev->stats.rx_errors++;
			goto free_entry;
		}
...
/**
 * eut_pull_set_descriptor() - Code for translating user request into EUT desc.
 * @pchan	Pull channel to set the descriptor for.
 ...
 * @return	0 on success, <0 on failure.
 */
static int eut_pull_set_descriptor(struct pp_pull_channel *pchan,
				   void *ctx,
				   size_t count,
				   struct pp_fifo_entry *entry)
{
	struct file *filp = (struct file *)ctx;
	struct eut_pull_descriptor *desc = (struct eut_pull_descriptor *)entry->desc;
	struct pp_file_ctx *fctx = filp->private_data;
...
/**
 * gnpunet_pull_set_descriptor() - Code for translating net request into NPU
 *     pull desc.
 * @pchan	Pull channel to set the descriptor for.
 * @ctx:	Context for initializing the descriptor.
 * @count:	The size of the data buffer.
 ...
 * @return	0 on success, <0 on failure.
 */
static int gnpunet_pull_set_descriptor(struct pp_pull_channel *pchan,
				       void *ctx,
				       size_t count,
				       struct pp_fifo_entry *entry)
{
...
/**
 * eutnet_pull_set_descriptor() - Code for translating net request into EUT desc
 * @pchan	Pull channel to set the descriptor for.
 * @ctx:	Context for initializing the descriptor.
 * @count:	The size of the data buffer.
 * @entry:	The fifo entry to use for setting up the descriptor.
 ...
 * @return	0 on success, <0 on failure.
 */
static int eutnet_pull_set_descriptor(struct pp_pull_channel *pchan,
				      void *ctx,
				      size_t count,
				      struct pp_fifo_entry *entry)
{
						 ...
						 u32 resource_idx)
{
	...
	rc = pp_chan_init_common(dev, of_node, chan, "tx", resource_idx,
				 desc_info->dynamic_pull_buffers, pp_pull_irq);
	if (rc) {
		dev_err(&dev->dev, "Failed common init (%d)\n", rc);
		goto free_channel;
	}
			 ...
			 loff_t *pos)
{
	...
	while (rc == 0) {
		...
		if ((entry->data_len > 0) && (entry->data_len <= count)) {
			...
			/* Report and continue. */
			if (WARN_ONCE(rc, "Failed to copy data to user: %zd",
				      rc)) {
				rc = -EFAULT;
			} else {
				rc = entry->data_len;
			  ...
			  const char __user *buf, size_t count, loff_t *pos)
{
	...
	if (WARN_ONCE(copy_from_user(entry->buffer, buf, count),
...
static const struct pp_desc_info pp_desc_list[] = {
	...
	{
		.name = "eut",
		.pull_encode_descriptor = eut_pull_set_descriptor,
		.push_extra_init = linkid_push_chan_init,
		.push_handle_pkt = cdec_push_handle_pkt,
		.if_register = pp_chrdev_register_lid,
		.if_unregister = pp_chrdev_unregister_device,
	},
	{
		...
		.pull_extra_init = linkid_net_pull_chan_init,
		.pull_extra_work = network_pull_extra_work,
		.pull_encode_descriptor = eutnet_pull_set_descriptor,
		.push_extra_init = net_push_chan_init,
		.push_extra_deinit = net_push_chan_deinit,
		.push_prepare_desc = pp_net_push_prepare_desc,
		.push_handle_pkt = cdecnet_push_handle_pkt,
	...
	},
	{
		...
		.pull_extra_init = net_pull_chan_init_common,
		.pull_extra_work = network_pull_extra_work,
		.pull_encode_descriptor = gnpunet_pull_set_descriptor,
		.push_extra_init = net_push_chan_init,
		.push_extra_deinit = net_push_chan_deinit,
		.push_prepare_desc = pp_net_push_prepare_desc,
		.push_handle_pkt = gnpunet_push_handle_pkt,
		.if_register = pp_register_net,
		.if_unregister = pp_unregister_net,
		.dynamic_push_buffers = true,
	},
...
static int pp_probe_common(struct pp_priv *priv, struct platform_device *dev)
{
	...
	BUILD_BUG_ON(sizeof(struct eut_pull_descriptor) !=
		     EUT_PULL_DESCRIPTOR_SIZE);
