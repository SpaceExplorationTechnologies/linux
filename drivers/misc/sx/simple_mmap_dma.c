...
static void dma_complete(void *p)
{
	 ...
	 */
	getrawmonotonic(&dma->transfer_start_time);
 ...
 * @return zero on success or a negative error code on a failure
 */
int smmap_dma_start_delay(struct smmap_dma *dma, const struct timespec *delay)
{
	if (!dma->timer.function)
		return -ENXIO;

	if (delay->tv_sec != 0 || !timespec_valid(delay)) {
		/* Don't allow delays > 1 second. */
		return -EINVAL;
	}

	hrtimer_start(&dma->timer, timespec_to_ktime(*delay),
			HRTIMER_MODE_REL);
	return 0;
}
