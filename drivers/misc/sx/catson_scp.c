 ...
 *
 * This driver only supports the RX and TXL FIFOs.
 */
 ...
 * @context	private context structure
 */
static void tx_reset(struct catscp_priv *priv)
{
	struct catscp_channel *chan = &priv->tx;
	u32 rd_idx;
	unsigned long flags;
	...
	catscp_reg_write(priv, SCP_MEM_TXL_FIFO_BA_REG_OFFSET,
			 (u32)chan->desc_phys);

	/* Reset all indexes to the RD_IDX location. */
	rd_idx = catscp_reg_read(priv, SCP_MEM_TXL_FIFO_RD_IDX_REG_OFFSET) &
				 SCP_MEM_TXL_FIFO_RD_IDX__TXL_RD_IDX_bm;

	catscp_reg_write(priv, SCP_MEM_TXL_FIFO_WR_IDX_REG_OFFSET, rd_idx);
	chan->desc_index = rd_idx;
	chan->last_complete_index = rd_idx;

	/* Enable TX flow. */
	catscp_reg_write(
	    priv, SCP_MEM_TXL_FIFO_FLAGS_REG_OFFSET,
 ...
 */
static bool tx_can_fill_work_fifo(struct catscp_priv *priv)
{
	struct catscp_channel *chan = &priv->tx;
	u32 wr_idx = catscp_reg_read(priv, SCP_MEM_TXL_FIFO_WR_IDX_REG_OFFSET) &
 ...
 */
static int catscp_fill_tx_fifo_thread(void *context)
{
	struct catscp_priv *priv = (struct catscp_priv *)context;
	struct catscp_channel *chan = &priv->tx;
	unsigned long flags;
	...
	while (!kthread_should_stop()) {
		 ...
		 * on to the work fifo.
		 */
		while (tx_can_fill_work_fifo(priv)) {
			struct catscp_fifo_entry entry;
			...
			if (link_is_up(priv) && priv->evt_msg_received) {
				...
				smp_mb();
				catscp_reg_write(priv,
						 SCP_MEM_TXL_FIFO_WR_IDX_REG_OFFSET,
						 chan->desc_index);
 ...
 * @return	zero on success, non-zero on failure
 *
 * This adds the scp device to the lookup table so that open() on
 * the character device works. Only one device is possible - there's
 * only one in the chip.
 */
int catscp_register_device(struct catscp_priv *priv)
{
	...
	if (priv->major == 0) {
		rc = -EBADF;
		dev_err(priv->dev, "Bad major (%d)\n", rc);
		goto out;
	}

	/* Hard-coded minor of 1, so only one instance is allowed. */
	dev_id = MKDEV(MAJOR(priv->major), 1);
	cdev_init(&priv->cdev, &catscp_fops);
	rc = cdev_add(&priv->cdev, dev_id, 1);
	if (rc) {
		dev_err(priv->dev, "Failed to add char device (%d)\n", rc);
		goto out;
	}

	device = device_create(catscp_class, NULL, dev_id, NULL, "catscp%d", i);
	if (IS_ERR(device)) {
		rc = PTR_ERR(device);
 ...
 */
void catscp_unregister_device(struct catscp_priv *priv)
{
 ...
 * @return	irq handled status
 */
static irqreturn_t catscp_tx_irq(struct catscp_priv *priv)
{
	struct catscp_channel *chan = &priv->tx;
	u32 desc_phys;
	u32 wr_idx;
	...
	wr_idx = catscp_reg_read(priv, SCP_MEM_TXL_FIFO_WR_IDX_REG_OFFSET) &
				 SCP_MEM_TXL_FIFO_WR_IDX__TXL_WR_IDX_bm;
	while (chan->last_complete_index != wr_idx) {
...
static irqreturn_t catscp_irq(int irq, void *d)
{
	...
	if (irq_stat & SCP_CMN_ITS__TXL_BUF_COMPLETE_ITS_bm) {
		if (catscp_tx_irq(priv) == IRQ_HANDLED)
			ret = IRQ_HANDLED;
	}
...
static int catscp_start(struct catscp_priv *priv)
{
	...
	priv->tx_work_thread = kthread_run(catscp_fill_tx_fifo_thread,
...
static struct attribute *catscp_sysfs_control_attrs[] = {
	&dev_attr_link_enable.attr, &dev_attr_link_speed.attr, NULL};
...
static int __init catscp_init(void)
{
	...
	rc = alloc_chrdev_region(&major, 0, 1, DRIVER_NAME);
	if (rc) {
		goto out;
	}
